...
def CIT(data, method='fisherz', **kwargs):
    ...
    if method == fisherz:
        ...
        return FCIT(data, **kwargs)
    elif method == ppi:
        return PPI(data, **kwargs)
    elif method == ppi2w:
        return PPI_2w(data, **kwargs)
    else:
...
# permutive predictive independence
class PPI(TransformerMixin, BaseEstimator, CIT_Base):
    def __init__(self, data, model="GB", n_estimators=300, num_simul=50, simul_size=0.2, simul_type=0, 
                 sig_test_type="non-parametric", random_state=2023, n_jobs=-1, verbose=True):
        
        super().__init__(data)
        
        self.check_cache_method_consistent('ppi', NO_SPECIFIED_PARAMETERS_MSG)
        self.assert_input_data_is_valid()
        self.data = data
        ...
        if model == "GB":
            self.reg = GradientBoostingRegressor(n_estimators=n_estimators,max_depth=5,
                                                 validation_fraction=0.1,
                                                 n_iter_no_change=5,
                                                 tol=0.0001,
                                                 random_state=self.random_state)
            self.clf = GradientBoostingClassifier(n_estimators=n_estimators,max_depth=5,
                                                  validation_fraction=0.1,
                                                  n_iter_no_change=5,
                                                  tol=0.0001,
                                                  random_state=self.random_state)
            ...
            self.reg = DecisionTreeRegressor(random_state=self.random_state)
    ...
    #---------------------------------------------------------------------------------------------
    # feature_importance_c() - Function that gives score with and without feature transformation #
    #---------------------------------------------------------------------------------------------
    ...
    def feature_importance(self, data, Y, i):
        ...
        if len(np.unique(Y)) > 5:
            ...
            # Fit the model on shuffled train data
            # model.fit(x_train, y_train)
            preds = model.predict(x_test)
    ...
    #-----------------------------------------------------
    # PPI() - Function that provides feature importance #
    #-----------------------------------------------------
    ...
    def __call__(self, X, Y, Z = None):
        ...
        X, Y, Z, cache_key = self.get_formatted_XYZ_and_cachekey(X, Y, Z)
        if cache_key in self.pvalue_cache: return self.pvalue_cache[cache_key]
        ...
        if Y.ndim == 1:
            Y = np.reshape(Y, (-1, 1))
            predictor = Y
            if len(np.unique(Y)) >= len(np.unique(X)): # and Y.shape[1] <= X.shape[1]
                target = Y
                predictor = X
        ...
        if Z is None:
            ...
            if Z.ndim == 1:
                ...
                features = np.concatenate((predictor, np.reshape(Z, (-1, 1))), axis=1)
            else:
                features = np.concatenate((predictor, Z), axis=1)

        parallel = Parallel(n_jobs=self.n_jobs, verbose=self.verbose)
        
        test1, test2 = list(), list()
        p_values = parallel(delayed(self.feature_importance)(features, target, i) for i in range(self.num_simul))
        p_values = np.array(p_values)
        test1, test2 = p_values[:,0], p_values[:,1]

        # Since we want the loss to be smaller for test1,
        # we therefore perform one tail paired t-test (to the left and not right)
        
        if self.sig_test_type == "parametric":
            t_stat, p_val = ss.ttest_ind(test1, test2, alternative="less", nan_policy="omit")
        else:
            t_stat, p_val = ss.wilcoxon(test1, test2, alternative="less", zero_method="zsplit")
        
        self.pvalue_cache[cache_key] = p_val
        
        return p_val #ss.t.cdf(t_stat, len(testX) + len(testY) - 2) # Left part of t-test
    
    
class PPI_2w(TransformerMixin, BaseEstimator, CIT_Base):
    def __init__(self, data, model="GB", n_estimators=300, num_simul=50, simul_size=0.2, simul_type=1, 
                 sig_test_type="non-parametric", random_state=2023, n_jobs=-1, verbose=True):
        
        super().__init__(data)
        
        self.check_cache_method_consistent('ppi2w', NO_SPECIFIED_PARAMETERS_MSG)
        self.assert_input_data_is_valid()
        self.data = data
    ...
    def feature_importance(self, data, Y, i):
        ...
        if len(np.unique(Y)) > 5:
            ...
            if self.simul_type == 0:
                ...
                x_train, x_test, y_train, y_test = train_test_split(data, Y, test_size=self.simul_size, random_state=i, stratify=Y)
            # Find all the labels. If there are more than 2 labels, then use multi-class classification
            # So handle accordingly
            self_labels = np.unique(y_train)
            if len(self_labels) <= 2:
                self_labels = None
    ...
    def __call__(self, X, Y, Z = None):
        ...
        if Z is None:
            ...
            if Z.ndim == 1:
                # Always keep the variable to check, in front
                features1 = np.concatenate((X, np.reshape(Z, (-1, 1))), axis=1)
                features2 = np.concatenate((Y, np.reshape(Z, (-1, 1))), axis=1)
            else:
                features1 = np.concatenate((X, Z), axis=1)
                features2 = np.concatenate((Y, Z), axis=1)

        # test1 -> prediction on correct set
        # test2 -> prediction on permuted set
        test1, test2 = list(), list()
        parallel = Parallel(n_jobs=self.n_jobs, verbose=self.verbose)
        p1_values = parallel(delayed(self.feature_importance)(features1, Y, i) for i in range(self.num_simul))
        p1_values = np.array(p1_values)
        test1, test2 = p1_values[:,0], p1_values[:,1]
        
        test3, test4 = list(), list()
        p2_values = parallel(delayed(self.feature_importance)(features2, X, i) for i in range(self.num_simul))
        p2_values = np.array(p2_values)
        test3, test4 = p2_values[:,0], p2_values[:,1]

        # Since we want the loss to be smaller for test1,
        # we therefore perform one tail paired t-test (to the left and not right)
        
        if self.sig_test_type == "parametric":
            t1_stat, p1_val = ss.ttest_ind(test1, test2, alternative="less", nan_policy="omit")
            t2_stat, p2_val = ss.ttest_ind(test3, test4, alternative="less", nan_policy="omit")
        else:
            t1_stat, p1_val = ss.wilcoxon(test1, test2, alternative="less", zero_method="zsplit")
            t2_stat, p2_val = ss.wilcoxon(test3, test4, alternative="less", zero_method="zsplit")
        
        p_val = min(p1_val, p2_val)
        # p_val = (p1_val + p2_val)/2
        self.pvalue_cache[cache_key] = p_val
